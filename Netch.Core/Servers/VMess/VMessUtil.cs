...
public class VMessUtil : IServerUtil
{
    ...
    {
        NetchAppContext.ServerEditor?.EditServer(s);
    }

    public void Create()
    {
        NetchAppContext.ServerEditor?.CreateServer(TypeName);
    }

    public string GetShareLink(Server s)
    {
        if (NetchAppContext.Current!.Settings.V2RayConfig.V2rayNShareLink)
        {
    ...
    {
        return new V2rayController(NetchAppContext.Current!);
    }
