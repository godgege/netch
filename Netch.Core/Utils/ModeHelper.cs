...
{
    /// <summary>
    ///     Base directory for mode files. Must be set at application startup.
    /// </summary>
    public static string? ModeBaseDirectory { get; set; }

    private static readonly JsonSerializerOptions JsonSerializerOptions = NetchAppContext.NewCustomJsonSerializerOptions();
    ...
    private static Mode ReadTxtMode(string file)
    {
        ...
        foreach (var l in ls.Skip(1))
        {
            ...
            if (l.StartsWith("#include"))
            {
                ...
                includeMode = ReadTxtMode(Path.Combine(ModeBaseDirectory!, relativePath));
            }
