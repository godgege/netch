...
public class Bandwidth
{
    ...
    public TraceEventSession? tSession;

    private static readonly string[] Suffix = { "B", "KiB", "MiB", "GiB", "TiB", "PiB", "EiB" };

    public Bandwidth(IStatusReporter statusReporter)
    {
    ...
    ///     根据程序名统计流量
    /// </summary>
    public void NetTraffic(IServerController? serverController, IModeController? modeController, CancellationToken ct)
    {
        ...
        _statusReporter.ReportBandwidthState(true);
        ...
        while (!ct.IsCancellationRequested)
        {
            Thread.Sleep(1000);
            lock (counterLock)
                _statusReporter.ReportBandwidthUpdated(received);
        }
    }
